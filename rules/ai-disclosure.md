...
| **Compilation** | latexmk, pandoc, knitr invocations |
...
### LaTeX pipeline (`paper/main.tex`)
Add `\section*{AI Use Statement}` immediately before `\clearpage \printbibliography`,
or in the Acknowledgements section using `\subsection*{AI Use Statement}`.

### Quarto pipeline (`manuscript_quarto.qmd`)
Place in the `## AI Use Statement {.unnumbered}` section at the end of the document.
This section is unnumbered (`{.unnumbered}`) and appears after References and before the Appendix.
